...
if (!Object.asWeakRef) {
  // @ts-ignore
  Object.prototype.asWeakRef = function (data: any) {
...
export const signalMemoryMap = () => {
  ...
  const weakMapObject: any[] = [];

  weakMap.forEach((_, key) => {
    const info = fromWeakRef(key, false);

    weakMapObject.push(
      { value: key, signal: info.signal, path: info.path },
    );
  });
  ...
  return {
    ...
    shared: Object.fromEntries(
      Object.entries(sharedObject).map((
        [key, signal],
      ) => [key, { value: signal.value, signal: signal }]),
    ),
    weak: weakMapObject,
  };
};
...
export const useComputed = <T extends any>(
  handle: (raw: <S>(v: S) => Signal.UnpackSignalRaw<S>) => T,
) => {
...
export const useLazyComputed = <T extends unknown>(
  models: Props.OrSignal<unknown>[],
  handle: (raw: <S>(v: S) => Signal.UnpackSignalRaw<S>) => T,
) => {
  return useSignal<T>(null as T, {
    onInit(signal) {
...
export const watch = <T extends unknown>(
  ...
  weakRef.signal[sWatch].set(key, (value, path) => {
    if (
      (path.length - 1) < weakRef.pathIndex ||
...
export const useStrongRef = <T extends unknown, K = T>(
  value: T | Signal.Signal<T, K>,
  handle: (raw: K, unwatch: () => void) => void,
  forceDeep = false,
) => {
  if (isSignal(value)) {
    // @ts-ignore
  ...
  const unwatch = watch(value, (val) => handle(val as any, unwatch), {
    flush: "sync",
    deep: forceDeep,
  });
  return unwatch;
...
export const unSignal = <T,>(value: T): Signal.UnpackSignalRaw<T> => {
