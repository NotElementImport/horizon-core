...
export const executeSync = (data: Record<string, unknown>, cacheControl?: Record<string, unknown>) => {
...
export const tryReadSharedData = <T extends unknown>(key: string|number, handle: (value: T) => void): void => {
    const tryRead = () => {
...
export const writeSharedData = <T extends unknown>(key: string|number, value: T) => {
    sharedStructure.data.set(key, value)
}

export const readSharedData = <T extends unknown>(key: string|number, $default: T = null as T) => {
...
export const useCacheControl = (config: Fetching.CacheControlConfig = {}): Fetching.HorizonFetchCacheControl => {
    ...
    if(shared) {
        return {
            write(key, data, time) {
                ...
                if(time) setTimeout(() => { sharedStructure.cacheControl.delete(key) }, time)
                return data
            },
            read(key) {
            ...
            forgetAll(startsWith) {
                if(!startsWith) throw new Error('Global cache cannot be without startWith')
        ...
        return {
            write(key, data, time) {
                ...
                local.set(key, data)
                if(time) setTimeout(() => { local.delete(key) }, time)
                return data
            },
            read(key) {
            ...
            tryRead(key, $default) {
                // @ts-ignore
                if(!isClient) return typeof $default == 'function' ? $default() : $default
